...
$runbooks = (Get-AzAutomationRunbook -AutomationAccountName $($automationaccounts.AutomationAccountName) -ResourceGroupName "$($automationaccounts.ResourceGroupName)")

  foreach($job in $runbooks){
   Start-AzAutomationRunbook -ResourceGroupName jwgovernance -AutomationAccountName wolffentpautoact -nAME $($JOB.NAME) -RunOn wolffhybridworkergroup -Verbose 
  }
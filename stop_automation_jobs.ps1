...
catch
{
 ...
 $running = (Get-AzAutomationJob -ResourceGroupName jwgovernance -AutomationAccountName wolffentpautoact  | where RunbookName -eq wolffStoragetop5BlobsRB | Select-Object JobId)

  foreach($job in $running){
   Stop-AzAutomationJob -ResourceGroupName jwgovernance -AutomationAccountName wolffentpautoact -Id $($Job.JobId) -Verbose 
  }